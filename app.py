...
if uploaded_file:
    with st.spinner("Processing uploaded file... please wait ⏳"):
        ...
        # Fix for missing column safely
        state_list = df.get("Student Doctor's State", pd.Series(dtype='str')).dropna().unique()
        state_filter = st.sidebar.multiselect("Filter by State", sorted(state_list))

        rbm_list = df.get("HQ Code", pd.Series(dtype='str')).dropna().unique()
        rbm_filter = st.sidebar.multiselect("Filter by RBM", sorted(rbm_list))
        ...
        if state_filter:
            df = df[df.get("Student Doctor's State", pd.Series(dtype='str')).isin(state_filter)]
        if rbm_filter:
            df = df[df.get('HQ Code', pd.Series(dtype='str')).isin(rbm_filter)]
        ...
        with tab7:
            ...
            if 'RBM' in execution_df.columns:
                ...
                st.warning("RBM data not available in the uploaded file.")
